...
{ // Based on the action selection method chosen, it selects an action to execute next
    if(action_sel==1) //Greedy, always selects the action with the largest Q value
    {
        ...
        for(int a=1; a<4; a++)
        {
            if(Qvalues[x_pos][y_pos][a] > max_value)
            {
                max_value = Qvalues[x_pos][y_pos][a];
                best_action = a;
            }
        }
        return best_action;
...
void move(int action)
{
    ...
    if(action==0)
    {
        if((y_pos<(height_grid-1))&&(blocked[x_pos][y_pos+1]==0))
        {
            y_pos=y_pos+1;
        }
    }
    if(action==1)
    {
        if((x_pos<(width_grid-1))&&(blocked[x_pos+1][y_pos]==0))
        {
            x_pos=x_pos+1;
        }
    }
    if(action==2)
    {
        if((y_pos>0)&&(blocked[x_pos][y_pos-1]==0))
        {
            y_pos=y_pos-1;
        }
    }
    if(action==3)
    {
        if((x_pos>0)&&(blocked[x_pos-1][y_pos]==0))
        {
            x_pos=x_pos-1;
        }
    }
...
{
    float max_q = *max_element(Qvalues[x_pos][y_pos], Qvalues[x_pos][y_pos]+4);
    Qvalues[prev_x_pos][prev_y_pos][action_taken] += learn_rate * (reward[x_pos][y_pos] + disc_factor * max_q - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
}

void update_q_prev_state_sarsa()
{
    Qvalues[prev_x_pos][prev_y_pos][action_taken] += learn_rate * (reward[x_pos][y_pos] + disc_factor * Qvalues[x_pos][y_pos][action_taken2] - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
}
...
int main(int argc, char* argv[])
{
    ...
    string alg_str = (algorithm == 1) ? "qlearning" : "sarsa";
    string env_str = (environment == 1) ? "ambiente1" : "ambiente2";
    string filename = alg_str + "_" + env_str + ".txt";

    reward_output.open(filename);

    reward_output << "# Algoritmo: " << ((algorithm == 1) ? "Q-learning" : "SARSA") << "\n";
    reward_output << "# Ambiente: " << ((environment == 1) ? "Pequeño" : "Cliff") << "\n";

    Initialize_environment(); // Inicializar ambiente
    ...
    for(i = 0; i < num_episodes; i++)
    {
        ...
        reward_output << i << " " << cum_reward << "\n";
        cout << " Total reward obtenido: " << cum_reward << "\n";
    }
